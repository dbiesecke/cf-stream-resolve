...
export function extractMedia(html: string, page: URL): ResolvedVideo | undefined {
  ...
  return { url: primary, source: meta(html, ["og:site_name"]) ?? page.hostname, original: page.href, thumbnail: safeAsset(meta(html, ["og:image", "twitter:image"]), page), favicon: safeAsset(faviconTag && attribute(faviconTag, "href"), page), alternates: unique.filter((value) => value !== primary) };
}
